...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
        ...
        ,'<p>'
          ...
          ,'{{# if(d.data){ }}'
            ,'{{# if(d.data.type === "group"){ }}'
              ,'<i class="layui-icon layim-chat-detail" layim-event="detail" data-index="'+'{{d.data.id}}'+'">&#xe613;</i>'
            ,'{{# } }}'
          ,'{{# } }}'
        ,'</p>'
      ,'</div>'
      ...
      ,'<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
       ...
       /* ,'{{# if(d.base && d.base.uploadFile){ }}'
        ,'<span class="layui-icon layim-tool-image" title="发送文件" layim-event="image" data-type="file">&#xe61d;<input type="file" name="file"></span>'
         ,'{{# }; }}'*/
        ,'{{# if(d.base && d.base.uploadFile){ }}'
        ,'<span class="layui-icon layim-tool-image" title="发红包" layim-event="redEnvelope" >&#xe667;</span>'
        ,'{{# }; }}'
        ,'{{# if(d.data.type=="friend"){ }}'
        ,'<span class="layui-icon layim-tool-image" title="转账" data-id="{{d.data.id}}" layim-event="transferMoney" >&#xe648;</span>'
        ,'{{# }; }}'
  ...
  layui.data.content = function(content){
    ...
    .replace(/red\[([^\s]+?)\]/g, function(red){  //转义红包
        var str    = red.replace(/(^red\[)|(\]$)/g, '');
        var id     = str.split('|')[0];
        var money  = str.split('|')[1];
        var src    = str.split('|')[2];
        return '<div class="red-envelope" data-number="'+money+'" data-id="'+id+'" onclick="redenvelope(this)">' +
            '<image class="layui-layim-red" src="'+src+'">' +
            '<div class="red-number" >'+money+'元</div>' +
            '</div>';
    })
    .replace(/transfer\[([^\s]+?)\]/g, function(transfer){  //转义转账
        var str      = transfer.replace(/(^transfer\[)|(\]$)/g, '');
        var money    = str.split('|')[0];
        var src      = str.split('|')[1];
        var nickname = '';
        if(str.split('|')[2]){
          nickname = ' 转给 '+str.split('|')[2];
        }
        return '<div class="red-envelope" data-number="'+money+'" >' +
            '<image class="layui-layim-red" src="'+src+'">' +
            '<div class="red-number" >'+money+'元 '+nickname+'</div>'
            '</div>';
    })
  ...
  var elemChatMain = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">'
    ,'<div class="layim-chat-user"><img src="{{ d.avatar }}" data-id="{{d.id}}" data-click=0 '
      ,'{{# if(!d.mine && d.type=="group"){ }}'
      ,'onclick="clickHaed(this)"'
      ,'{{# }; }}'
      ,'><cite>'
      ,'{{ d.username||"佚名" }}'
    ,'</cite></div>'
  ...
  var cache = {message: {}, chat: []}, init = function(options){
    ...
    ,obj = {
      ...
      //从数据库获取历史记录
      $.ajax({
          url:"/index/chatlog/historyJson",
          type:"POST",
          data:{id:mine.id},
          async:false,
          success:function (data) {
              obj.history = data.msg;
          }
      });
  ...
  var viewChatlog = function(){
    ...
    var thatChat = thisChat(), chatlog = local.chatlog || {};
      //查找数据库聊天记录
      $.ajax({
          url:"/index/chatlog/chatlogJson",
          type:"POST",
          data:{id:cache.mine.id},
          async:false,
          success:function (data) {
              chatlog = data.msg;
          }
  ...
  var anim = 'layui-anim-upbit', events = { 
    ...
    //红包
    ,redEnvelope: function(othis,e){
      var content = '', thatChat = thisChat(), input = thatChat.textarea;
      content+='红包金额<input type="number" value="" style="height: 30px ;width: 155px"><button class="layui-btn layui-btn-sm" id="money" style="margin-left: 8px">确定</button><button class="layui-btn layui-btn-sm layui-btn-danger" id="close"style="margin-left: 8px">取消</button>';
      content = '<div class="layui-layim-redEnvelope">'+ content +'</div>';
      layer.popBottom({
          content: content
          ,success: function(){
          }
      ...
      $('#money').click(function (events) {
        var number = $('[type=number]').val();
        if(number>0){
            $.post("/index/redenvelope/sendRed",
                {
                    money:number,
                },
                function(data){
                  if(data.code){
                    src='/static/layui/images/red/wait.png';
                    str=data.code+'|'+number+'|'+src;
                    focusInsert(input[0], 'red['+ str +']');
                    input.next()[input.val() === '' ? 'addClass' : 'removeClass']('layui-disabled');
                    sendMessage();
                  }else{
                    layer.msg(data.msg)
    ...
    //转账
    ,transferMoney: function(othis,e){
      var content = '', thatChat = thisChat(), input = thatChat.textarea;
      content+='转账金额<input type="number" value="" style="height: 30px ;width: 155px"><button class="layui-btn layui-btn-sm" id="money" style="margin-left: 8px">确定</button><button class="layui-btn layui-btn-sm layui-btn-danger" id="close"style="margin-left: 8px">取消</button>';
      content = '<div class="layui-layim-redEnvelope">'+ content +'</div>';
      layer.popBottom({
          content: content
          ,success: function(){
      ...
      $('#money').click(function (events) {
        var number = $('[type=number]').val();
        if(number>0){
            $.post("/index/redenvelope/transferMoney",
                {
                ...
                function(data){
                  if(data.code){
                    src='/static/layui/images/red/transfer.png';
                    str= number+'|'+src;
                    if(data.data){
                      str += '|'+data.data;
                    }
                    focusInsert(input[0], 'transfer['+ str +']');
                    input.next()[input.val() === '' ? 'addClass' : 'removeClass']('layui-disabled');
                    sendMessage();
                  }
                  layer.msg(data.msg)
    ...
    ,group: function(){
      popPanel({
        title: '群聊'
        ,tpl: ['<li class="layim-add" layim-event="addGroup"><i class="layui-icon">&#xe654;</i>创建群聊</li><div class="layui-layim-list layim-list-group">'
          ,listTpl({
            type: 'group'
            ,item: 'd.group'
          })
    ...
    ,addGroup: function(){
      window.location.href = '/index/findgroup/addgroup'
    }

    //查看群组成员
    ,detail: function(othis){
      window.location.href = '/index/findgroup/checkgroup?id='+othis.attr('data-index');
    ...
    ,about: function(){
      layer.open({
        content: '<p style="padding-bottom: 5px;">1</p><p>当前版本：1.0</p>'
...
function redenvelope(thisObj){
    obj = $(thisObj);
    var id = obj.attr('data-id');
    $.post("/index/redenvelope/isopen",
        {
            id:id,
        },
        function(data){
            if(data.code==0){
                layer.msg(data.msg);
                obj.children('img').attr('src','/static/layui/images/red/gone.png') ;
                return false;
            }else{
                layer.confirm('确认打开红包？',function (index) {
                    $.post("/index/redenvelope/receiveRed",
                        {
                            id:id,
                        },
                        function(data){
...
function clickHaed(thisObj){
    obj = $(thisObj);
    var id = obj.attr('data-id');
    var is_click = obj.attr('data-click');
    if(is_click==0){
        var html = "<span style='position:absolute;z-index:999;'><div class='layui-btn layui-btn-sm add-friend' data-id='"+id+"'  onclick='addFriend(this)'>添加好友</div>" +
            "<br><div class='layui-btn layui-btn-sm add-friend' style='margin-top: 1px' data-id='"+id+"' layim-event='transferMoney'>向他转账</div></span>";
        obj.after(html);
